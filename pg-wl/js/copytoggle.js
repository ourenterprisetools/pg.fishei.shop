...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const ref_id = qs.get('ref_id');
  const ttclid = qs.get('ttclid');

  document.querySelectorAll('a.buylink').forEach(a => {
    try {
      const u = new URL(a.href);
      if (cid)    u.searchParams.set('cid', cid);           // necessário pro postback IC/Purchase
      if (ref_id) u.searchParams.set('ref_id', ref_id);     // opcional (debug/consistência)
      if (ttclid && !u.searchParams.has('ttclid')) u.searchParams.set('ttclid', ttclid); // opcional
      a.href = u.toString();
    } catch(e) {}
  });
});